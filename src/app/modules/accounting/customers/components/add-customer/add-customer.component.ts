...
import { AddEditSupplier } from '../../../suppliers/shared/data/add-edit-supplier.model';
import { ActivatedRoute, Router } from '@angular/router';
import { NgxUiLoaderService } from 'ngx-ui-loader';
import { AddEditCustomer } from '../../shared/data/customer.addEdit';
...
export class AddCustomerComponent implements OnInit {
  ...
  breadCrumbItems: BreadCrumbItem[] = [
    ...
    {url:'', label:'sideNav.cutomersManagement.addCustomer'}
  ];
  ...
  autoCoding: string;
  attchement: File;
  addEditModel: AddEditCustomer;
  alertInput: AlertInput = new AlertInput();
  imageSrc: string = '/assets/images/icons/thumbnail.jpg';
  constructor(private customerService:CustomerService,private formBuilder: FormBuilder, private activatedRoute: ActivatedRoute, private router: Router, private ngxService: NgxUiLoaderService) {
...
/*       const formData: FormData = new FormData();
     ...
     if(this.attchement!=null)
       formData.append('file',this.attchement,this.attchement.name); */
        this.customerService.addEdit( this.addEditForm.value).subscribe(res=> {
          this.clearForm();
...
/*     const formData: FormData = new FormData();
   ...
   if(this.attchement!=null)
      ...
      */
      this.customerService.addEdit(this.addEditForm.value).subscribe(res=> {
      //  this.router.navigate(['/'+FullRoutes.STOCK_MODULE+'/'+FullRoutes.CUSTOMER_MODULE+'/'+FullRoutes.CUSTOMER_LIST]); 
        this.ngxService.stopBackground();
        this.alertInput = new AlertInput("s", "shared.savedsuccess");
  ...
  cancal(){
    this.router.navigate(['/'+FullRoutes.SALES_MODULE+'/'+FullRoutes.CUSTOMER_MODULE+'/'+FullRoutes.CUSTOMER_LIST]);
  }
  findbyId(id)
  {
    this.ngxService.start();
    this.customerService.findById(id)
    .subscribe((res: AddEditCustomer) => {
  ...
  getdealingNaturity() {
    this.customerService.getLookup('dealingNaturity')
  ...
  getCountries() {
    this.customerService.getLookup('country')
      .subscribe((res: BaseObject[]) => {
        this.countries = res;
        // this.distributors = res;
        // console.log(res)
  ...
  getCites(parentId) {
    ...
    this.customerService.getByParent(parentId)
      .subscribe((res: BaseObject[]) => {
        this.cites = res;
        // this.distributors = res;
        // console.log(res)
  ...
  getpaymentCondition() {
    this.customerService.getLookup('paymentCondition')
      .subscribe((res: BaseObject[]) => {
  ...
  getSystemCoding() {
    this.customerService.getSystemParameter('systemCoding')
